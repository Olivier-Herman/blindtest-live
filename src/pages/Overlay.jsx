  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: 'transparent', overflow: 'hidden', position: 'relative', fontFamily: "'Orbitron', monospace" }}>
      ...
      <div style={{ position: 'absolute', top: '4vh', left: 0, right: 0, textAlign: 'center', zIndex: 20 }}>
        <div style={{ fontWeight: 900, fontSize: '4.5vw', letterSpacing: '0.4em', color: '#ff2d78', textShadow: '0 0 20px #ff2d78, 0 0 50px rgba(255,45,120,.4)', animation: 'titleIn 1s ease' }}>
          ♪ BLIND TEST ♪
        </div>
      </div>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 20 }}>
        {gameState.status !== 'revealed' ? (
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '15vw', fontWeight: 900, color: '#ff2d78', animation: 'qPulse 2s ease-in-out infinite', lineHeight: 1 }}>?</div>
            <div style={{ fontSize: '2.2vw', color: 'rgba(255,255,255,.4)', fontFamily: 'Share Tech Mono', letterSpacing: '0.5em', marginTop: '1.5vw', animation: 'fadeSlide .8s ease' }}>
            ...
            {gameState.status === 'idle' && (
              <div style={{ marginTop: '2vw', fontSize: '1.6vw', color: 'rgba(255,255,255,.2)', fontFamily: 'Share Tech Mono', letterSpacing: '0.4em' }}>
          ...
          <div style={{ textAlign: 'center', animation: 'revealIn .7s cubic-bezier(.34,1.56,.64,1)', padding: '0 5vw' }}>
            <div style={{ fontSize: '1.6vw', color: 'rgba(255,255,255,.4)', fontFamily: 'Share Tech Mono', letterSpacing: '0.6em', marginBottom: '1.5vw' }}>
              ✦ LA RÉPONSE ÉTAIT ✦
            </div>
            <div style={{ fontSize: '6.5vw', fontWeight: 900, color: '#00f5ff', textShadow: '0 0 20px #00f5ff, 0 0 60px rgba(0,245,255,.4)', lineHeight: 1.1, marginBottom: '0.8vw' }}>
              {gameState.song_title}
            </div>
            <div style={{ fontSize: '2.8vw', color: 'rgba(255,255,255,.5)', fontFamily: 'Share Tech Mono', letterSpacing: '0.2em' }}>
              — {gameState.song_artist}
            </div>
            {gameState.winner_name && (
              <div style={{ marginTop: '2.5vw', display: 'inline-block', padding: '1vw 3vw', background: 'rgba(255,215,0,.1)', border: '1px solid rgba(255,215,0,.5)', borderRadius: '5vw', animation: 'winnerIn .5s ease .4s both' }}>
                <span style={{ fontSize: '2.2vw', color: '#ffd700', fontWeight: 900, letterSpacing: '0.15em', textShadow: '0 0 12px #ffd700' }}>
      ...
      {gameState.status === 'playing' && (
        <div style={{
          ...
          borderRadius: '1.5vw',
          padding: '1vw 2.5vw',
          boxShadow: `0 0 25px ${timerColor}50, 0 0 60px ${timerColor}20`,
          animation: timer <= 7 ? 'timerPulse .6s ease-in-out infinite' : 'fadeSlide .4s ease',
          minWidth: '13vw',
          textAlign: 'center'
        }}>
          <div style={{ fontSize: '1.1vw', color: 'rgba(255,255,255,.4)', fontFamily: 'Share Tech Mono', letterSpacing: '0.4em', marginBottom: '0.3vw' }}>TEMPS</div>
          <div style={{ fontSize: '24vw', fontWeight: 900, color: timerColor, textShadow: `0 0 20px ${timerColor}, 0 0 50px ${timerColor}50`, lineHeight: 1 }}>
            {timer}
          </div>
          <div style={{ fontSize: '1.1vw', color: 'rgba(255,255,255,.3)', fontFamily: 'Share Tech Mono', letterSpacing: '0.3em', marginTop: '0.2vw' }}>sec</div>
        </div>
      )}
      ...
      {scores.length > 0 && (
        <div style={{
          ...
          <div style={{ fontSize: '1.1vw', color: 'rgba(255,255,255,.4)', fontFamily: 'Share Tech Mono', letterSpacing: '0.4em', marginBottom: '1vw', textAlign: 'center' }}>
            🏆 TOP {Math.min(scores.length, 5)}
          </div>
          {scores.slice(0, 5).map((p, i) => (
            <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', gap: '2vw', fontSize: '1.8vw', fontFamily: 'Share Tech Mono', marginBottom: '0.6vw', color: i === 0 ? '#ffd700' : i === 1 ? '#c0c0c0' : i === 2 ? '#cd7f32' : 'rgba(255,255,255,.55)' }}>
      ...
      {gameState.round_number > 0 && (
        <div style={{ position: 'absolute', top: '4vh', right: '3vw', fontSize: '1.3vw', color: 'rgba(255,255,255,.25)', fontFamily: 'Share Tech Mono', letterSpacing: '0.3em', zIndex: 20 }}>
