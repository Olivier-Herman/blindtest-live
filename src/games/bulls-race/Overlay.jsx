...
export default function BullsRaceOverlay() {
  ...
  const isRevealed = state.status === 'revealed' || state.status === 'duel_result'
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: '#000', overflow: 'hidden', position: 'relative', fontFamily: "'Orbitron', monospace" }}>
      ...
      {/* ═══ JEU NORMAL ═══ */}
      {!isPodium && state.status !== 'rules' && state.status !== 'waiting' && state.status !== 'duel' && state.status !== 'duel_result' && (
        <>
          {/* Header */}
